...
int loadClutter(char * filename, SsFloat radius, struct site tx) {
     ...
     */
    int x, y, z, h = 0, w = 0;
    SsFloat clh, xll, yll, cellsize, cellsize2, xOffset, yOffset, lat, lon;
    char line[100000];
    char *s, *pch = NULL;
    FILE * fd;
...
int loadLIDAR(char *filenames, int resample, struct output *out)
{
    ...
    SsFloat avgCellsize = 0, smCellsize = 0;
    tile_t *tiles;
    ...
    SsFloat total_height = out->max_north - out->min_north;
    if (G_debug) {
    ...
    size_t new_width = 0;
    for (size_t i = 0; i < (unsigned)fc; i++) {
        SsFloat north_offset = out->max_north - tiles[i].max_north;
        SsFloat west_offset = out->max_west - tiles[i].max_west >= 0 ? out->max_west - tiles[i].max_west
...
int LoadTopoData(SsFloat max_lon,
                 SsFloat min_lon,
                 SsFloat max_lat,
                 SsFloat min_lat,
                 struct output * out) {
...
int LoadUDT(char * filename) {
    ...
    SsFloat latitude, longitude, height, tempheight, old_longitude = 0.0,
                                                     old_latitude = 0.0;
    ...
    do {
        ...
        if (z == 0) {
            ...
            AddElevation((SsFloat)xpix * G_dpp, (SsFloat)ypix * G_dpp, height, 1);
        }
