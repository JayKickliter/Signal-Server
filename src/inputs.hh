...
int
resample_data(int scaling_factor);
int
resize_data(int resolution);

int
LoadSDF_SDF(char * name, int winfiles);
char *
BZfgets(char * output, BZFILE * bzfd, unsigned length);
int
LoadSDF_GZ(char * name);
char *
GZfgets(char * output, gzFile gzfd, unsigned length);
int
LoadSDF_BZ(char * name);
int
LoadSDF(char * name, int winfiles);
int
...
int
loadLIDAR(char * filename, int resample, struct output * out);
int
loadClutter(char * filename, double radius, struct site tx);
int
                  averageHeight(int h, int w, int x, int y);
