/*****************************************************************************
 *  COST231-HATA MODEL for Signal Server by Alex Farrant * 30 December 2013i
...
SsFloat COST231pathLoss(float f, float TxH, float RxH, float d, int mode) {
    /*
    ...
    */
    /*	if (f < 150 || f > 2000) {
                    fprintf
                        (stderr,"Error: COST231 Hata model frequency range
       150-2000MHz\n"); exit(EXIT_FAILURE);
            }
    */
    ...
    float logf = log10(f);
    SsFloat dbloss = c0 + (cf * logf) - (13.82 * log10(TxH)) - C_H
