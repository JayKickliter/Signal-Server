        ...
        if (prop.tgh > prop.cch) { /* for TX ant above all clutter height */
            ...
            if (d1a < 50.0) {
                ...
                if (d1a < 225.0) {
                    ...
                    arte = q
                           + (0.7 * pdk - mymax(0.01, log10(prop.wn * 47.7) - 2))
                                 * (prop.hg[1] / hone);
                }
            ...
            q = q
                + (17.98 - 0.84224 * (prop.cch - prop.tgh))
                      * exp(-0.00000061 * pd);
...
double adiff2(double d, prop_type & prop, propa_type & propa) {
    ...
    sf2 =
        1.0; /* average empirical hilltop foliage scatter factor for 2 obstructions */
...
double alos(double d, prop_type & prop, propa_type & propa) {
    ...
    if (d == 0.0) {
        ...
        alosv =
            (-4.343 * log(abq_alos(complex<double>(cos(q), -sin(q)) + r)) - alosv)
                * wls
            + alosv;
    }
    return alosv;
    ...
    if (d == 0.0) {
        ...
        if (prop.mdp < 0) {
            ...
            if ((sps < 0.05) && (prop.cch > hrg)
                && (prop.dist
                    < prop.dl[0])) { /* if far from transmitter and receiver below canopy */
...
double
avar(double zzt, double zzl, double zzc, prop_type & prop, propv_type & propv) {
    static __thread int    kdv;
    static __thread double dexa, de, vmd, vs0, sgl, sgtm, sgtp, sgtd, tgtd, gm,
...
*************************************************************************************************/
{
    ...
    if (q <= 0.0) {
        ja =
            (long)(3.0 + 0.1 * elev[0]); /* to match KD2BD addition of (long) */
        jb = np - ja + 6;
        for (i = ja - 1; i < jb; ++i)
            zsys += elev[i];
        zsys /= (jb - ja + 1);
