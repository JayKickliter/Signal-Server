...
namespace {
    ...
    bool ***        processed;
    bool            has_init_processed =
        true; // XXX hacked for now because we don't use threads
    ...
    void beginThread(void * arg) {
        ...
        int rc =
            pthread_create(&threads[thread_count], NULL, rangePropagation, arg);
        if (rc)
...
PlotLOSPath(struct output * out,
    ...
    for (x = 0; (bStop == false) && (x < (out->path.length - 1))
                && (out->path.distance[x] <= lr->max_range);
         x++) {
        if (x > 0) {
        ...
        if ((cos_horizon_angle >= cos_angle)
            && ((GetMask(out, out->path.lat[x], out->path.lon[x]) & mask_value)
                == 0)
...
PlotPropPath(struct output * out,
    ...
    for (x = 1; x < out->path.length - 1; x++)
        out->elev[x + 2] =
            (out->path.elevation[x] == 0.0
                 ? out->path.elevation[x] * METERS_PER_FOOT
    ...
    for (y = 2;
         (y < (out->path.length - 1) && out->path.distance[y] <= lr->max_range);
         y++) {
        /* Process this point only if it
           has not already been processed. */

        if ((GetMask(out, out->path.lat[y], out->path.lon[y]) & 248)
                != (mask_value << 3)
            ...
            PutMask(out,
                    out->path.lat[y],
                    out->path.lon[y],
                    (GetMask(out, out->path.lat[y], out->path.lon[y]) & 7)
                        + (mask_value << 3));
        }
    }

    if (out->path.lat[y] > out->cropLat)
        out->cropLat = out->path.lat[y];
...
PlotPath(struct output * out,
    ...
    for (y = 0; y < out->path.length; y++) {
        ...
        if ((GetMask(out, out->path.lat[y], out->path.lon[y]) & mask_value) == 0) {
            ...
            if (block == 0)
                OrMask(out, out->path.lat[y], out->path.lon[y], mask_value);
        }
    }
}
