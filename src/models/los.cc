...
struct propagationRange {
    float min_west, max_west, min_north, max_north;
    float altitude;
...
void * rangePropagation(void * parameters) {
    ...
    float minwest = G_dpp + (float)v->min_west;
    float lon = v->eastwest ? minwest : v->min_west;
    float lat = v->min_north;
    int y = 0;
    ...
    do {
        ...
        ++y;
        if (v->eastwest) {
            lon = minwest + (G_dpp * (float)y);
        } else {
            lat = (float)v->min_north + (G_dpp * (float)y);
        }

    } while (v->eastwest ? (LonDiff(lon, (float)v->max_west) <= 0.0)
                         : (lat < (float)v->max_north));
 ...
 * effect to obstacles.
 */
static float ked(float freq, float rxh, float dkm, float * elev) {
...
void PlotLOSPath(output * out,
    ...
    bool bStop;
    int x, iCounter;
    float cos_angle, cos_test_angle, cos_horizon_angle, cos_limit_angle, rx_alt2;
    float distance, rx_alt, tx_alt, limit_alt, distance2, tx_alt2, test_alt,
...
void PlotLOSMap(struct output * out,
    ...
    float range_min_west[] = {out->min_west,
                               out->min_west,
                               out->min_west,
                               out->max_west};
    float range_min_north[] = {out->max_north,
                                out->min_north,
                                out->min_north,
                                out->min_north};
    float range_max_west[] = {out->max_west,
                               out->min_west,
                               out->max_west,
                               out->max_west};
    float range_max_north[] = {out->max_north,
                                out->max_north,
                                out->min_north,
                                out->max_north};
