...
struct propagationRange {
    SsFloat min_west, max_west, min_north, max_north;
    SsFloat altitude;
...
void * rangePropagation(void * parameters) {
    ...
    SsFloat minwest = G_dpp + (SsFloat)v->min_west;
    SsFloat lon = v->eastwest ? minwest : v->min_west;
    SsFloat lat = v->min_north;
    int y = 0;
    ...
    do {
        ...
        ++y;
        if (v->eastwest) {
            lon = minwest + (G_dpp * (SsFloat)y);
        } else {
            lat = (SsFloat)v->min_north + (G_dpp * (SsFloat)y);
        }

    } while (v->eastwest ? (LonDiff(lon, (SsFloat)v->max_west) <= 0.0)
                         : (lat < (SsFloat)v->max_north));
...
bool can_process(SsFloat lat, SsFloat lon) {
 ...
 * distance (adj)
 */
static SsFloat incidenceAngle(SsFloat opp, SsFloat adj) {
 ...
 * effect to obstacles.
 */
static SsFloat ked(SsFloat freq, SsFloat rxh, SsFloat dkm, SsFloat * elev) {
    SsFloat obh, obd, rxobaoi = 0, d;
...
void PlotLOSPath(output * out,
    ...
    bool bStop;
    int x, iCounter;
    SsFloat cos_angle, cos_test_angle, cos_horizon_angle, cos_limit_angle, rx_alt2;
    SsFloat distance, rx_alt, tx_alt, limit_alt, distance2, tx_alt2, test_alt,
        test_alt2, limit_alt2;
    ...
    limit_alt = EARTHRADIUS_FT + 32808.0;
    limit_alt2 = limit_alt * limit_alt;

    tx_alt = EARTHRADIUS_FT + source.alt + path.elevation[0];
    tx_alt2 = tx_alt * tx_alt;

    for (x = 0; (bStop == false) && (x < (path.ssize() - 1))
                && (path.distance[x] <= lr->max_range);
         x++) {
        if (x > 0) {
            distance = FEET_PER_MILE * path.distance[x];
            distance2 = distance * distance;

            rx_alt = EARTHRADIUS_FT + destination.alt + path.elevation[x];
            ...
            cos_angle =
                (distance2 + tx_alt2 - rx_alt2) / (2.0 * distance * tx_alt);
            ...
            cos_test_angle =
                (distance2 + tx_alt2 - test_alt2) / (2.0 * distance * tx_alt);
        } else {
            cos_angle = -1.0;
            cos_test_angle = 1.0;
        }
        ...
        if ((cos_horizon_angle >= cos_angle)
            && ((GetMask(out, path.lat[x], path.lon[x]) & mask_value) == 0)
            && (can_process(path.lat[x], path.lon[x]))) {
            OrMask(out, path.lat[x], path.lon[x], mask_value);
        }
        ...
        if ((x > 0) && (cos_horizon_angle < 0.0)) {
            if (iCounter > 10) {
                ...
                cos_limit_angle = (distance2 + tx_alt2 - limit_alt2)
                                  / (2.0 * distance * tx_alt);
...
void PlotPropPath(struct output * out,
    ...
    SsFloat loss, azimuth,
        pattern = 0.0, xmtr_alt, dest_alt, xmtr_alt2, dest_alt2, cos_rcvr_angle,
        cos_test_angle = 0.0, test_alt, elevation = 0.0, distance = 0.0,
        four_thirds_earth, field_strength = 0.0, rxp, dBm, diffloss;
    struct site temp;
    float dkm;
    ...
    for (x = 1; x < path.ssize() - 1; x++) {
        out->elev[x + 2] =
            (path.elevation[x] == 0.0
                 ? path.elevation[x] * METERS_PER_FOOT
                 : (lr->clutter + path.elevation[x]) * METERS_PER_FOOT);
    }
    ...
    for (y = 2; (y < (path.ssize() - 1) && path.distance[y] <= lr->max_range); y++) {
        /* Process this point only if it
           has not already been processed. */

        if ((GetMask(out, path.lat[y], path.lon[y]) & 248) != (mask_value << 3)
            ...
            /* Determine attenuation for each point along the
               path using a prop model starting at y=2 (number_of_points = 1),
               the shortest distance terrain can play a role in path loss. */
            ...
            case 6:
                // COST231-Hata
                loss = COST231pathLoss(lr->frq_mhz,
                                       source.alt * METERS_PER_FOOT,
                                       (path.elevation[y] * METERS_PER_FOOT)
                                           + (destination.alt * METERS_PER_FOOT),
                                       dkm,
            ...
            case 9:
                // Ericsson
                loss = EricssonpathLoss(lr->frq_mhz,
                                        source.alt * METERS_PER_FOOT,
                                        (path.elevation[y] * METERS_PER_FOOT)
                                            + (destination.alt * METERS_PER_FOOT),
                                        dkm,
            ...
            if (lr->erp != 0.0) {
                if (lr->dbm) {
                    ...
                    ofs = GetSignal(out, path.lat[y], path.lon[y]);
                    ...
                    field_strength = (139.4 + (20.0 * log10(lr->frq_mhz)) - loss)
            ...
            PutMask(out,
                    path.lat[y],
                    path.lon[y],
                    (GetMask(out, path.lat[y], path.lon[y]) & 7)
                        + (mask_value << 3));
        }
    }

    if (path.lat[y] > out->cropLat) {
        out->cropLat = path.lat[y];
    }
...
void PlotLOSMap(struct output * out,
    ...
    if (plo_filename && plo_filename && plo_filename[0] != 0) {
    ...
    SsFloat range_min_west[] = {out->min_west,
                                out->min_west,
                                out->min_west,
                                out->max_west};
    SsFloat range_min_north[] = {out->max_north,
                                 out->min_north,
                                 out->min_north,
                                 out->min_north};
    SsFloat range_max_west[] = {out->max_west,
                                out->min_west,
                                out->max_west,
                                out->max_west};
    SsFloat range_max_north[] = {out->max_north,
                                 out->max_north,
                                 out->min_north,
                                 out->max_north};
...
void PlotPropagation(struct output * out,
    ...
    if (G_debug) {
        fprintf(stderr,
                " contours of \"%s\" out to a radius of %.2f %s with Rx "
                "antenna(s) at %.2f %s AGL\n",
                source.name,
                lr->metric ? lr->max_range * KM_PER_MILE : lr->max_range,
                lr->metric ? "kilometers" : "miles",
                lr->metric ? altitude * METERS_PER_FOOT : altitude,
    ...
    if (lr->clutter > 0.0 && G_debug) {
        fprintf(stderr,
                ...
                lr->metric ? lr->clutter * METERS_PER_FOOT : lr->clutter,
                lr->metric ? "meters" : "feet");
    }
...
void PlotPath(Path const & path,
    ...
    char block;
    int x, y;
    SsFloat cos_xmtr_angle, cos_test_angle, test_alt;
    SsFloat distance, rx_alt, tx_alt;

    for (y = 0; y < path.ssize(); y++) {
        /* Test this point only if it hasn't been already
           tested and found to be free of obstructions. */

        if ((GetMask(out, path.lat[y], path.lon[y]) & mask_value) == 0) {
            distance = FEET_PER_MILE * path.distance[y];
            tx_alt = EARTHRADIUS_FT + source.alt + path.elevation[0];
            rx_alt = EARTHRADIUS_FT + destination.alt + path.elevation[y];
            ...
            if (block == 0) {
                OrMask(out, path.lat[y], path.lon[y], mask_value);
            }
        }
    }
}
