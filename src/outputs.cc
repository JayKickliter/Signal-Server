    ...
    for (y = 0, lat = out->north; y < (int)out->height;
        ...
        for (x = 0, lon = out->max_west; x < (int)out->width;
            ...
            for (auto & i : out->dem_out) {
                ...
                y0 = G_mpi
                     - (int)rint(G_ppd * (LonDiff((double)i.max_west, lon)));
    ...
    for (y = 0, lat = out->north; y < (int)out->height;
        ...
        for (x = 0, lon = out->max_west; x < (int)out->width;
            ...
            if (found) {
                mask = found->mask[DEM_INDEX(found->dem->ippd, x0, y0)];
                signal =
                    (found->signal[DEM_INDEX(found->dem->ippd, x0, y0)]) - 100;
                cityorcounty = 0;
                match        = 255;
                ...
                if (cityorcounty == 0) {
                    if (lr.contour_threshold != 0
                        && signal < lr.contour_threshold) {
                        if (ngs)
                            ADD_PIXELA(&ctx, 255, 255, 255, 0);
                        else {
                            /* Display land or sea elevation */
    ...
    if (lr.frq_mhz > 0.0) {
        ...
        for (x = 1; x < out->path.length - 1; x++)
            out->elev[x + 2] = METERS_PER_FOOT
                               * (out->path.elevation[x] == 0.0
                                      ? out->path.elevation[x]
                                      : (lr.clutter + out->path.elevation[x]));

        /* Copy ending points without clutter */

        out->elev[2] = out->path.elevation[0] * METERS_PER_FOOT;
        out->elev[out->path.length + 1] =
            out->path.elevation[out->path.length - 1] * METERS_PER_FOOT;

        azimuth = rint(Azimuth(source, destination));

        for (y = 2; y < (out->path.length - 1);
            ...
            dest_alt =
                four_thirds_earth + destination.alt + out->path.elevation[y];
            ...
            if (G_got_elevation_pattern) {
                ...
                for (x = 2, block = 0; x < y && block == 0; x++) {
                    distance = FEET_PER_MILE
                               * (out->path.distance[y] - out->path.distance[x]);
            ...
            out->elev[1] = METERS_PER_MILE
                           * (out->path.distance[y] - out->path.distance[y - 1]);
            ...
            case 3:
                // HATA 1, 2 & 3
                out->loss =
                    HATApathLoss(lr.frq_mhz,
                                 source.alt * METERS_PER_FOOT,
                                 (out->path.elevation[y] * METERS_PER_FOOT)
                                     + (destination.alt * METERS_PER_FOOT),
                                 dkm,
                                 pmenv);
                break;
...
SeriesData(struct site     source,
    ...
    if (G_debug) {
        fprintf(stderr,
                "SeriesData: az = %lf, dist = %lf, ref = %lf, b = %lf\n",
                azimuth,
                distance,
                refangle,
                b);
    ...
    for (x = 0; x < out->path.length - 1; x++) {
        ...
        if (lr.metric) {
            ...
            if (has_clutter && x > 0 && x < out->path.length - 2) {
                out->cluttervec.push_back(
                    METERS_PER_FOOT
                    * (terrain == 0.0 ? height : (height + lr.clutter)));
            }
        ...
        if ((height + lr.clutter) > maxheight)
            maxheight = height + lr.clutter;
    ...
    if (lr.metric) {
        out->distancevec.push_back(KM_PER_MILE
                                   * out->path.distance[out->path.length - 1]);
