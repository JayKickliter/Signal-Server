...
float G_dpp, G_ppd, G_yppd, G_fzone_clearance = 0.6, G_delta = 0;
...
float Distance(site const & site1, site const & site2) {
    ...
    float lat1, lon1, lat2, lon2, distance;
...
float Azimuth(site const & source, site const & destination) {
    ...
    float dest_lat, dest_lon, src_lat, src_lon, beta, azimuth, diff, num, den,
        fraction;
...
Path::Path(site const & src, site const & dst) {
    ...
    int c;
    float azimuth, distance_, lat1, lon1, beta, den, num, lat2, lon2,
    ...
    for (distance_ = 0.0, c = 0;
         ...
         c++, distance_ = miles_per_sample * (float)c) {
...
float ElevationAngle2(Path const & path,
    ...
    int x;
    char block = 0;
    float source_alt, destination_alt, cos_xmtr_angle, cos_test_angle, test_alt,
        elevation, distance, source_alt2, first_obstruction_angle = 0.0;
...
float ReadBearing(char * input) {
    /* This function takes numeric input in the form of a character
       ...
       decimal number (float).  The input may either be expressed
    ...
    if (b == 2) { /* Degree, Minute, Second Format (40 08 23.xx) */
        ...
        bearing = fabs((float)degrees);
        bearing += fabs(((float)minutes) / 60.0);
...
void ObstructionAnalysis(Path const & path,
    ...
    int x;
    struct site site_x;
    float h_r, h_t, h_x, h_r_orig, cos_tx_angle, cos_test_angle, cos_tx_angle_f1,
...
int handle_args(int argc, char * argv[], output & out) {
    ...
    float min_lat, min_lon, max_lat, max_lon, rxlat, rxlon, txlat, txlon,
    ...
    float altitude = 0.0, altitudeLR = 0.0, tx_range = 0.0, rx_range = 0.0,
           deg_range = 0.0, deg_limit = 0.0, deg_range_lon;

    float tercon, terdic, rxGain = 0, antenna_rotation, antenna_downtilt,
    ...
    min_lon = (float)floor(out.tx_site[0].lon);
    max_lon = (float)floor(out.tx_site[0].lon);
    ...
    /*if (lidar) {
        ...
        G_ppd = ((float)out.height / (out.max_north - out.min_north));
        G_yppd = G_ppd;
