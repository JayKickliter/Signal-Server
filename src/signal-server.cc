...
SsFloat G_dpp, G_ppd, G_yppd, G_fzone_clearance = 0.6, G_delta = 0;
...
SsFloat arccos(SsFloat x, SsFloat y) {
        ...
        for (i = size * -1; i <= size; i = i + 1) {
            for (j = size * -1; j <= size; j = j + 1) {
                if (x + j >= 0 && x + j <= IPPD && y + i >= 0 && y + i <= IPPD)
                    ...
                    found->data[DEM_INDEX(found->ippd, y + i, x + j)] +=
                        (short)rint(height);
            }
        }
    }
...
SsFloat dist(SsFloat lat1, SsFloat lon1, SsFloat lat2, SsFloat lon2) {
...
SsFloat Distance(site const & site1, site const & site2) {
    ...
    SsFloat lat1, lon1, lat2, lon2, distance;
...
SsFloat Azimuth(site const & source, site const & destination) {
    ...
    SsFloat dest_lat, dest_lon, src_lat, src_lon, beta, azimuth, diff, num, den,
        fraction;
...
SsFloat ElevationAngle(site const & source, site const & destination) {
    ...
    return ((180.0 * (acos(((b * b) + (dx * dx) - (a * a)) / (2.0 * b * dx))) / PI)
            - 90.0);
}
...
Path::Path(site const & src, site const & dst) {
    ...
    int c;
    SsFloat azimuth, distance_, lat1, lon1, beta, den, num, lat2, lon2,
    ...
    for (distance_ = 0.0, c = 0;
         (total_distance != 0.0 && distance_ <= total_distance && c < ARRAYSIZE);
         c++, distance_ = miles_per_sample * (SsFloat)c) {
    ...
    assert(lat.size() == lon.size() && lon.size() == elevation.size()
           && elevation.size() == distance.size());
}

ssize_t Path::ssize() const {
    return lat.size();
}
...
SsFloat ElevationAngle2(Path const & path,
    ...
    int x;
    char block = 0;
    SsFloat source_alt, destination_alt, cos_xmtr_angle, cos_test_angle,
        test_alt, elevation, distance, source_alt2, first_obstruction_angle = 0.0;
    ...
    for (x = 2, block = 0; x < path.ssize() && block == 0; x++) {
        ...
        test_alt = EARTHRADIUS_FT
                   + (path.elevation[x] == 0.0 ? path.elevation[x]
                                               : path.elevation[x] + lr.clutter);
...
SsFloat ReadBearing(char * input) {
    /* This function takes numeric input in the form of a character
       ...
       decimal number (SsFloat).  The input may either be expressed
    ...
    if (b == 0) { /* Decimal Format (40.139722) */
        sscanf(string, "%f", &bearing);
    }

    if (b == 2) { /* Degree, Minute, Second Format (40 08 23.xx) */
        sscanf(string, "%d %d %f", &degrees, &minutes, &seconds);

        bearing = fabs((SsFloat)degrees);
        bearing += fabs(((SsFloat)minutes) / 60.0);
...
void ObstructionAnalysis(Path const & path,
    ...
    for (x = path.ssize() - 1; x > 0; x--) {
            ...
            if (site_x.lat >= 0.0) {
                if (lr.metric) {
                    fprintf(outfile,
                            ...
                            METERS_PER_FOOT * (h_x - EARTHRADIUS_FT));
                } else {
                    fprintf(
                        outfile,
        ...
        if (f) {
            ...
            cos_tx_angle_f1 = ((h_r_f1 * h_r_f1) + (d_tx * d_tx) - (h_t * h_t))
                              / (2.0 * h_r_f1 * d_tx);
            ...
            h_f = h_los
                  - G_fzone_clearance * sqrt(lambda * d_x * (d_tx - d_x) / d_tx);
    ...
    if (h_r > h_r_orig) {
        if (lr.metric) {
            snprintf(string,
                     ...
                     rcvr.name,
                     METERS_PER_FOOT * (h_r - GetElevation(rcvr) - EARTHRADIUS_FT));
        } else {
            snprintf(string,
                     150,
                     ...
                     "AGL\nto clear all obstructions detected.\n",
                     rcvr.name,
                     h_r - GetElevation(rcvr) - EARTHRADIUS_FT);
        }
    }
        ...
        if (h_r_fpt6 > h_r_orig) {
            if (lr.metric) {
                snprintf(
                    ...
                    rcvr.name,
                    METERS_PER_FOOT
                        * (h_r_fpt6 - GetElevation(rcvr) - EARTHRADIUS_FT),
                    G_fzone_clearance * 100.0,
                    37);

            } else {
                snprintf(string_fpt6,
                         150,
                         "\nAntenna at %s must be raised to at least %.2f feet "
                         "AGL\nto clear %.0f%c of the first Fresnel zone.\n",
                         rcvr.name,
        ...
        if (h_r_f1 > h_r_orig) {
            if (lr.metric) {
                snprintf(string_f1,
                         ...
                         rcvr.name,
                         METERS_PER_FOOT
                             * (h_r_f1 - GetElevation(rcvr) - EARTHRADIUS_FT));
...
void resize_elev(struct output & out) {
    out.elev.resize(ARRAYSIZE + 10, 0.0);
}
...
int handle_args(int argc, char * argv[], output & out) {
    ...
    SsFloat min_lat, min_lon, max_lat, max_lon, rxlat, rxlon, txlat, txlon,
        west_min, west_max, nortRxHin, nortRxHax;

    bool use_threads = false;

    unsigned char LRmap = 0, txsites = 0, topomap = 0, geo = 0, kml = 0,
                  area_mode = 0, max_txsites, ngs = 0;

    char clutter_file[255], antenna_file[255];
    char *az_filename, *el_filename, *udt_file = NULL;

    SsFloat altitude = 0.0, altitudeLR = 0.0, tx_range = 0.0, rx_range = 0.0,
            deg_range = 0.0, deg_limit = 0.0, deg_range_lon;

    SsFloat tercon, terdic, rxGain = 0, antenna_rotation, antenna_downtilt,
                            antenna_dt_direction;
    ...
    for (x = 0; x <= y; x++) {
        ...
        if (strcmp(argv[x], "-o") == 0) {
            ...
            if (z <= y && argv[z][0] && argv[z][0] != '-') {
                ...
                if ((az_filename =
                         (char *)calloc(strlen(argv[z]) + strlen(AZ_FILE_SUFFIX) + 1,
                                        sizeof(char)))
                    == NULL) {
                    return ENOMEM;
                }
                ...
                if ((el_filename =
                         (char *)calloc(strlen(argv[z]) + strlen(EL_FILE_SUFFIX) + 1,
                                        sizeof(char)))
                    == NULL) {
                    free(az_filename);
                    return ENOMEM;
                ...
                if ((result = LoadPAT(az_filename, el_filename, lr)) != 0) {
                    fprintf(stderr,
                            "Permissions error reading antenna pattern file\n");
                    free(az_filename);
                    free(el_filename);
                    exit(result);
                }
        ...
        /*if (strcmp(argv[x], "-lid") == 0) {
            z = x + 1;
            lidar = 1;
            if (z <= y && argv[z][0] && argv[z][0] != '-') strncpy(lidar_tiles,
        argv[z], 27000);  // 900 tiles!
        }*/
        ...
        /*if (strcmp(argv[x], "-resample") == 0) {
            ...
            if (!lidar) {
                fprintf(stderr, "Error, this should only be used with LIDAR
        tiles.\n"); return -1;
            }
        ...
        if (strcmp(argv[x], "-lon") == 0) {
            ...
            if (z <= y && argv[z][0]) {
                ...
                out.tx_site[0].lon *= -1;
                if (out.tx_site[0].lon < 0.0) {
                    out.tx_site[0].lon += 360.0;
                }
            }
        }
        ...
        if (strcmp(argv[x], "-rlo") == 0) {
            ...
            if (z <= y && argv[z][0]) {
                ...
                out.tx_site[1].lon *= -1;
                if (out.tx_site[1].lon < 0.0) {
                    out.tx_site[1].lon += 360.0;
                }
            }
        }
    ...
    min_lon = (SsFloat)floor(out.tx_site[0].lon);
    max_lon = (SsFloat)floor(out.tx_site[0].lon);
    ...
    /*if (lidar) {
        ...
        G_ppd = ((SsFloat)out.height / (out.max_north - out.min_north));
        G_yppd = G_ppd;

        if (G_debug) {
            fprintf(stderr, "ppd %f, yppd %f, %.4lf,%.4lf,%.4lf,%.4lf,%d x
    %d\n", G_ppd, G_yppd, out.max_north, out.min_west, out.min_north,
    out.max_west, out.width, out.height); fflush(stderr);
        }
    ...
    if (area_mode || topomap) {
        for (z = 0; z < txsites && z < max_txsites; z++) {
            ...
            tx_range =
                sqrt(1.5 * (out.tx_site[z].alt + GetElevation(out.tx_site[z])));
    ...
    if (strlen(clutter_file) > 1) {
        /*
        Clutter tiles cover 16 x 12 degs but we only need a fraction of that
        area. Limit by max_range / miles per degree (at equator)
        */
        if ((result = loadClutter(clutter_file, lr.max_range / 45, out.tx_site[0]))
            != 0) {
    ...
    if (ppa == 0) {
        if (propmodel == 2) { // Model 2 = LOS
            cropping =
                false; // TODO: File is written in DoLOS() so this needs moving
                       // to PlotPropagation() to allow styling, cropping etc
            ...
            if (cropping) {
                ...
                out.max_west =
                    out.cropLon + out.tx_site[0].lon; // degrees west (positive)
                out.cropLat -= out.tx_site[0].lat;    // angle from tx to edge

                if (G_debug) {
                    fprintf(stderr,
                            "Cropping 1: max_west: %.4f cropLat: %.4f cropLon: "
                            "%.4f longitude: %.5f dpp %.7f\n",
                            out.max_west,
                            out.cropLat,
                            out.cropLon,
                            out.tx_site[0].lon,
                ...
                if (G_debug) {
                    fprintf(stderr,
                            "Cropping 2: max_west: %.4f cropLat: %.4f cropLon: "
                            "%.7f longitude: %.5f width %d\n",
                            out.max_west,
                            out.cropLat,
                            out.cropLon,
                            out.tx_site[0].lon,
                ...
                if (out.width > 3600 * 10 || out.cropLon < 0) {
                    fprintf(stderr,
                            "FATAL BOUNDS! max_west: %.4f cropLat: %.4f "
                            "cropLon: %.7f longitude: %.5f\n",
                            out.max_west,
                            out.cropLat,
                            out.cropLon,
                            out.tx_site[0].lon);
        ...
        if (out.tx_site[0].lon < -180.0) {
            out.tx_site[0].lon += 360;
        }
