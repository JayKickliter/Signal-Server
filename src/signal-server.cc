...
ElevationAngle2(struct site     source,
    ...
    int    x;
    char   block = 0;
    double source_alt, destination_alt, cos_xmtr_angle, cos_test_angle, test_alt,
    ...
    for (x = 2, block = 0; x < out->path.length && block == 0; x++) {
        ...
        test_alt = G_earthradius
                   + (out->path.elevation[x] == 0.0
                          ...
                          : out->path.elevation[x] + lr.clutter);
            ...
            if (site_x.lat >= 0.0) {
                if (lr.metric)
                    fprintf(outfile,
                            ...
                            METERS_PER_FOOT * (h_x - G_earthradius));
                else
                    fprintf(
                        outfile,
    ...
    if (h_r > h_r_orig) {
        if (lr.metric)
            snprintf(string,
                     ...
                     rcvr.name,
                     METERS_PER_FOOT * (h_r - GetElevation(rcvr) - G_earthradius));
        else
            snprintf(string,
                     150,
        ...
        if (h_r_fpt6 > h_r_orig) {
            if (lr.metric)
                snprintf(
                    ...
                    rcvr.name,
                    METERS_PER_FOOT
                        * (h_r_fpt6 - GetElevation(rcvr) - G_earthradius),
        ...
        if (h_r_f1 > h_r_orig) {
            if (lr.metric)
                snprintf(string_f1,
                         ...
                         rcvr.name,
                         METERS_PER_FOOT
                             * (h_r_f1 - GetElevation(rcvr) - G_earthradius));
...
int
handle_args(int argc, char * argv[], output & out) {
    /* Scan for command line arguments */
    int x, y, z = 0, propmodel, knifeedge = 0, ppa = 0, normalise = 0, haf = 0,
              pmenv = 1, result;
    ...
    double tercon, terdic, rxGain = 0, antenna_rotation, antenna_downtilt,
    ...
    for (x = 0; x <= y; x++) {
        ...
        if (strcmp(argv[x], "-o") == 0) {
            ...
            if (z <= y && argv[z][0] && argv[z][0] != '-') {
                ...
                if ((az_filename = (char *)calloc(strlen(argv[z])
                                                      + strlen(AZ_FILE_SUFFIX) + 1,
                                                  sizeof(char)))
                    == NULL)
                    return ENOMEM;
                if (antenna_file[0] != '\0')
                ...
                if ((el_filename = (char *)calloc(strlen(argv[z])
                                                      + strlen(EL_FILE_SUFFIX) + 1,
                                                  sizeof(char)))
                    == NULL) {
                    free(az_filename);
                    return ENOMEM;
    ...
    /*if (lidar) {
        ...
        if (G_debug) {
            fprintf(stderr, "ppd %lf, yppd %lf, %.4lf,%.4lf,%.4lf,%.4lf,%d x
    ...
    if (area_mode || topomap) {
        ...
        if ((result = LoadTopoData(max_lon, min_lon, max_lat, min_lat, &out))
            != 0) {
            // This only fails on errors loading SDF tiles
            fprintf(stderr, "Error loading topo data\n");
            return result;
