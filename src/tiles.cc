...
SsFloat haversine_formula(SsFloat th1, SsFloat ph1, SsFloat th2, SsFloat ph2) {
...
int tile_load_lidar(tile_t * tile, char * filename, struct output * out) {
    ...
    if (tile->xur > out->eastoffset) {
        out->eastoffset = tile->xur;
    }
    if (tile->xll < out->westoffset) {
        out->westoffset = tile->xll;
    }
    ...
    if (G_debug) {
        fprintf(stderr,
                "POST yll %.7f yur %.7f xur %.7f xll %.7f delta %.6f\n",
                tile->yll,
                tile->yur,
                tile->xur,
                tile->xll,
    ...
    if ((tile->data = (short *)calloc(tile->width * tile->height, sizeof(short)))
        == NULL) {
        fclose(fd);
        free(tile->filename);
        return ENOMEM;
    ...
    SsFloat current_res_km = haversine_formula(tile->max_north,
                                               tile->max_west,
                                               tile->max_north,
                                               tile->min_west);
    tile->precise_resolution =
        (current_res_km / MAX(tile->width, tile->height) * 1000);

    // Round to nearest 0.5
    tile->resolution = tile->precise_resolution < 0.5f
                           ? 0.5f
                           : ceil((tile->precise_resolution * 2) + 0.5) / 2;

    // Positive westing
    tile->width_deg = tile->max_west - tile->min_west >= 0
                          ? tile->max_west - tile->min_west
                          : tile->max_west + (360 - tile->min_west);
    ...
    if (G_debug) {
        fprintf(stderr,
                "Pixels loaded: %zu/%d (PPD %dx%d, Res %f (%.2f))\n",
                loaded,
...
int tile_rescale(tile_t * tile, float scale) {
    ...
    /* Allocate the array for the lidar data */
    if ((new_data = (short *)calloc(new_height * new_width, sizeof(short)))
        == NULL) {
        return ENOMEM;
    }
    ...
    for (size_t y = 0, j = 0; y < (unsigned)tile->height && j < new_height;
         y += skip_count, j += copy_count) {
        for (size_t x = 0, i = 0; x < (unsigned)tile->width && i < new_width;
            ...
            for (size_t copy_y = 0; copy_y < copy_count; copy_y++) {
                for (size_t copy_x = 0; copy_x < copy_count; copy_x++) {
                    ...
                    /* Do the copy */
                    new_data[new_j * new_width + new_i] =
    ...
    if (G_debug) {
        fprintf(stderr,
                "Resampling complete. New resolution: %.1f\n",
                tile->resolution);
    }
...
int tile_resize(tile_t * tile, int resolution) {
    ...
    if (G_debug) {
        fprintf(stderr,
                "Resampling: Current %dm Desired %dm Scale %.1f\n",
                current_res,
                resolution,
                scaling_factor);
    }
