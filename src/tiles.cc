...
int tile_load_lidar(tile_t *tile, char *filename, struct output *out)
{
    ...
    if (G_debug)
        fprintf(stderr, "%d, %d, %.7f, %.7f, %.7f, %.7f, %.7f\n", tile->width, tile->height, tile->xll, tile->yll,
                tile->cellsize, tile->yur, tile->xur);
...
int tile_rescale(tile_t *tile, float scale)
{
    ...
    if (G_debug) fprintf(stderr, "Resampling complete. New resolution: %.1f\n", tile->resolution);
...
int tile_resize(tile_t *tile, int resolution)
{
    ...
    if (G_debug) fprintf(stderr, "Resampling: Current %dm Desired %dm Scale %.1f\n", current_res, resolution, scaling_factor);
